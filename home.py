...
def selectVideo():
    global left_frame, vid_label, vid_read
    for wid in right_frame.winfo_children():
        wid.destroy()

    vidfiletype = [("Video", "*mp4. *.webm *.ogg")]
    path = filedialog.askopenfilename(title="Choose a Video file", filetypes=vidfiletype)

    if(len(path) > 0):
        vid_read = cv2.imread(path)

        vid_size =  left_frame.winfo_height() - 40
        showImage(img_read, img_size)

##video File page##
def getPage4():
    global active_page, left_frame, right_frame, vid_label, heading
    img_label = None
    active_page = 4
    pages[4].lift()
    ...
    basicPageSetup(4)
    heading.configure(text="Detect Criminal")
    right_frame.configure(text="Detected Criminals through Video file")

    btn_grid = tk.Frame(left_frame, bg="#202d42")
    btn_grid.pack()

    tk.Button(btn_grid, text="Select Video File", command=selectVideo, font="Arial 15 bold", padx=20, bg="#2196f3",
            fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#091428",
            activeforeground="white").grid(row=0, column=0, padx=25, pady=25)
    tk.Button(btn_grid, text="Recognize", command=startRecognition, font="Arial 15 bold", padx=20, bg="#2196f3",
           fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#091428",
...
tk.Button(btn_frame, text="Detect throught Video file", command=getPage4)
